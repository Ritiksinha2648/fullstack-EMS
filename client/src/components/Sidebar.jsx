...
const Sidebar = () => {
                ...
                <div className='flex items-center justify-between'>
                    ...
                    <div className='flex items-center gap-3'>
                        ...
                        <div>
                         ...
                         text-white tracking-wide'>Employee MS</p>
                            <p className='text-[11px] 
                        text-slate-500 font-medium'>Management System</p>
                        </div>
                    </div>
 ...
 bg-white/3 border border-white/4 '>
                    <div className='flex items-center gap-3'>
                        ...
                        rounded-lg bg-slate-800 ring-1 ring-white/10 shrink-0'>
                        ...
                        <div className='min-w-0'>
                            ...
                            <p className='text-[11px] text-slate-500 truncate'>
                                {role === "ADMIN" ? "Administator" : "Employeee"}</p>
                        </div>
 ...
 tracking-[0.12em] text-slate-500'>Navigation</p>
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        ...
                        transition-all duration-150 relative
                         ${isActive ? "bg-indigo-500/12 text-indigo-300" :
                                "text-slate-300 hover:text-white hover:bg-white/4"
                            }`}>
                            {isActive &&
                                <div className='absolute left-0 top-1/2 
                        -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-indigo-500' />}
                            <item.Icon className={`w-[17px] h-[17px] shrink-0 
                         ${isActive ? "text-indigo-300"
                                    : "text-slate-400 group-hover:text-slate-300"}`} />

                            <span className='flex-1'>{item.name}</span>
                            {isActive && <ChevronRightIcon className='
                                    w-3.5 h-3.5 text-indigo-500/50'/>}
                        </Link>
                    )
                })}
    ...
    return (
        ...
        <>
         ...
         bg-slate-900 text-white rounded-lg border border-white/10'>
                <MenuIcon size={20} />
            </button>
            {/* Mobile overlay */}
 ...
 backdrop-blur-sm z-40'  onClick={() => setMobileOpen(false)} />}
            ...
            {/* sidebar -Desktop */}
            <aside className='hidden lg:flex flex-col h-full w-[260px] bg-linear-to-b from-slate-900
via-slate-900 to-slate-950 text-white shrink-0 border-r border-white/40'>
                {sidebarContent}
            </aside>
            ...
            <aside className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b
             from-slate-900 via-slate-900 to-slate-950 text-white z-50
