...
const EmployeeCard = ({ employee, onDelete, onEdit }) => {
    ...
    return (
     ...
     text-xs font-semibold text-slate-600 rounded-lg shadow-sm">
                    ...
                    {employee.isDeleted &&
                        <span className="bg-red-500/60 font-medium text-white 
                    px-2.5 py-1 text-xs rounded"> DELETED</span>}
                </span>
    ...
    group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6 gap-3">
                    <button onClick={() => onEdit(employee)} className="p-2.5 bg-white/90 backdrop-blur-sm text-slate-700
...
hover:text-rose-600 rounded-xl shadow-lg transition-all hover:scale-105 disabled:opacity-50">
