...
const autoCheckOut = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        if (!attendance?.checkOut) {
            ...
            await step.sleepUntil("wait-for-the-one-hours", new Date(new Date().getTime() + 1 * 60 * 60 * 1000))
...
const leaveApplicationRemainder = inngest.createFunction(
    {
        id: "leave-application-remainder", triggers: [
            {
                event: "leave/pending",
            },
        ],
    ...
    async ({ event, step }) => {
        ...
        if (leaveApplication?.status === "PENDING") {
            ...
            // send remainder email to admin to take action on leave  application
            ...
            await sendEmail({
                ...
                <p style="font-size: 18px; font-weight: bold; color: #007bff; margin: 8px 0;">
                ${leaveApplication?.startDate?.toLocaleDateString()}</p>
...
const attendanceRemainderCron = inngest.createFunction(
    {
        id: "attendance-remainder-cron", triggers: [
            {
    ...
    return {
        ...
        const absentEmployee = activeEmployees.filter((emp) =>
        ...
        if (absentEmployee.length > 0) {
            ...
            await step.run("send-remainder-emails", async () => {
                ...
                return {emailsSent : absentEmployee.length}
            });
